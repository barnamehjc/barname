    ...
    if (req.method === 'GET') {
      ...
      req.on('end', async () => {
        try {
          ...
          const prevUser = db.users[postUser] || {};
          const nextRevision = (prevUser.revision || 0) + 1;
          ...
          res.end(JSON.stringify({ success: true, user: postUser, revision: nextRevision, timestamp: parsed.lastModified || parsed.lastServerSync }));
        } catch (err) {
          res.writeHead(400, { 'Content-Type': 'application/json' });
